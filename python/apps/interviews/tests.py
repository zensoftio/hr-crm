...
from .serializers import CriteriaSerializer, InterviewerSerializer, InterviewerDetailSerializer
...
class InterviewDetailTest(GetInstanceTestMixin, APITestCase):
    url = '/interviews/'
    model = Interview
    serializer = InterviewerDetailSerializer

    fixtures = ['candidates.json', 'departments.json', 'requests.json', 'users.json', 'interviews.json', 'vacancies.json']

    def setUp(self):
        self.instance = Interview.objects.get(pk=2)
